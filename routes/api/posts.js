...
router.post(
  ...
  (req, res) => {
    ...
    Post.findById(req.params.id)
      .then(post => {
        console.log("At least post found");
        const newComment = {
          user: req.user.id,
          text: req.body.text,
          name: req.user.name,
      ...
      .catch(err =>
        res.status(404).json({
          postnotfound: "No Post Found" + err
        })
      );
  }
);

// @route   DELETE api/posts/comment/:id/:comment_id(post id and comment id that to be deleted)
      ...
      .then(post => {
        ...
        if (post.comments.filter(comment => comment._id.toString() === req.params.comment_id).length === 0) {
          return res.status(404).json({
            nocommentfound: `Comment does not exist`
          });
        }
